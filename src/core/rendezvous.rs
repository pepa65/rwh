...
pub enum RendezvousError {
	...
	#[error("Server wants one of {:?} for permissions, but we don't suppport any of these", _0)]
	Login(Vec<String>),
	#[cfg(not(target_family = "wasm"))]
	/// Websocket I/O error
...
impl RendezvousError {
	...
	pub(self) fn invalid_message(expected: &str, got: impl std::fmt::Debug) -> Self {
		Self::protocol(format!("Unexpected message (protocol error): Expected '{expected}', but got: {got:?}"))
	}
...
impl WsConnection {
	#[cfg(not(target_family = "wasm"))]
	async fn send_message(&mut self, message: &OutboundMessage, queue: Option<&mut MessageQueue>) -> Result<(), RendezvousError> {
		tracing::debug!("Sending {}", message);
		self.connection.send(ws2::Message::text(serde_json::to_string(message).unwrap())).await?;
	...
	async fn send_message(&mut self, message: &OutboundMessage, queue: Option<&mut MessageQueue>) -> Result<(), RendezvousError> {
		tracing::debug!("Sending {:?}", message);
		self.connection.send(ws_stream_wasm::WsMessage::Text(serde_json::to_string(message).unwrap())).await?;
		self.receive_ack(queue).await?;
		Ok(())
	}

	async fn receive_ack(&mut self, mut queue: Option<&mut MessageQueue>) -> Result<(), RendezvousError> {
		loop {
			...
			match message {
				...
				Some(InboundMessage::Message(message)) => match &mut queue {
					...
					None => {
						return Err(RendezvousError::protocol("Received peer message, but haven't opened the mailbox yet"));
					}
				},
				Some(other) => {
					return Err(RendezvousError::protocol(format!("Got unexpected message type from server '{other}'")));
				}
				None => continue,
			}
		}
	...
	async fn receive_reply(&mut self, mut queue: Option<&mut MessageQueue>) -> Result<RendezvousReply, RendezvousError> {
		loop {
			let message = self.receive_message().await?;
			match message {
				...
				Some(InboundMessage::Message(message)) => match &mut queue {
					...
					None => {
						break Err(RendezvousError::protocol("Received peer message, but haven't opened the mailbox yet"));
					}
				},
				...
				Some(other) => {
					break Err(RendezvousError::protocol(format!("Got unexpected message type from server '{other}'")));
				}
				None => (/*continue*/),
			}
		}
	...
	async fn receive_message(&mut self) -> Result<Option<InboundMessage>, RendezvousError> {
		let message = self.connection.next().await.expect("TODO this should always be Some")?;
		match message {
	...
	async fn receive_message(&mut self) -> Result<Option<InboundMessage>, RendezvousError> {
		...
		match message {
			...
			ws_stream_wasm::WsMessage::Binary(_) => Err(RendezvousError::protocol("WebSocket messages must be UTF-8 encoded text")),
		}
	}
...
impl std::fmt::Debug for RendezvousServer {
	fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		fmt.debug_struct("RendezvousServer").field("state", &self.state).field("side", &self.side).finish()
	}
}
...
impl RendezvousServer {
	...
	pub async fn connect(appid: &AppID, relay_url: &str) -> Result<(Self, Option<String>), RendezvousError> {
		...
		#[cfg(target_arch = "wasm32")]
		{
			...
			connection = WsConnection { meta, connection: stream };
		}

		let welcome = match connection.receive_message_some().await? {
			InboundMessage::Welcome { welcome } => welcome,
			other => {
				return Err(RendezvousError::protocol(format!("First message server sends must be 'welcome', but was '{other}'")));
			}
		};

		match welcome.permission_required {
			Some(PermissionRequired { hashcash: Some(hashcash), .. }) => {
				let token = crate::util::hashcash(hashcash.resource, hashcash.bits);
				connection
					.send_message(&OutboundMessage::SubmitPermission(SubmitPermission::Hashcash { stamp: token.to_string() }), None)
					.await?;
			}
		...
		connection.send_message(&OutboundMessage::bind(appid.clone(), side.clone()), None).await?;
	...
	async fn send_message(&mut self, message: &OutboundMessage) -> Result<(), RendezvousError> {
		self.connection.send_message(message, self.state.as_mut().map(|state| &mut state.queue)).await
	}

	async fn receive_reply(&mut self) -> Result<RendezvousReply, RendezvousError> {
		self.connection.receive_reply(self.state.as_mut().map(|state| &mut state.queue)).await
	}

	pub(crate) async fn send_peer_message(&mut self, phase: Phase, body: Vec<u8>) -> Result<(), RendezvousError> {
		self.send_message(&OutboundMessage::Add { body, phase }).await
	}

	pub(crate) async fn next_peer_message_some(&mut self) -> Result<EncryptedMessage, RendezvousError> {
		loop {
	...
	pub(crate) async fn next_peer_message(&mut self) -> Result<Option<EncryptedMessage>, RendezvousError> {
		let machine = &mut self.state.as_mut().expect("Can only receive messages when having a claimed+open mailbox");
		...
		match self.connection.receive_message().await? {
			...
			Some(other) => Err(RendezvousError::protocol(format!("Expected message from peer, got '{other}' instead"))),
			None => Ok(None),
		}
	}
	...
	pub async fn allocate_claim_open(&mut self) -> Result<(Nameplate, Mailbox), RendezvousError> {
		...
		self.send_message(&OutboundMessage::claim(nameplate.to_string())).await?;
		...
		self.send_message(&OutboundMessage::open(mailbox.clone())).await?;

		self.state =
			Some(MailboxMachine { nameplate: Some(nameplate.clone()), mailbox: mailbox.clone(), queue: Default::default(), processed: Default::default() });
	...
	pub async fn release_nameplate(&mut self) -> Result<(), RendezvousError> {
		...
		self.send_message(&OutboundMessage::release(nameplate.to_string())).await?;
	...
	pub async fn open_directly(&mut self, mailbox: Mailbox) -> Result<(), RendezvousError> {
		assert!(self.state.is_none(), "Can only call in initial state, and only once");
		self.send_message(&OutboundMessage::open(mailbox.clone())).await?;
		self.state = Some(MailboxMachine { nameplate: None, mailbox, queue: Default::default(), processed: Default::default() });
		Ok(())
	}

	pub async fn shutdown(mut self, mood: Mood) -> Result<(), RendezvousError> {
		if let Some(MailboxMachine { nameplate, mailbox, mut queue, .. }) = self.state {
			if let Some(nameplate) = nameplate {
				self.connection.send_message(&OutboundMessage::release(nameplate.to_string()), Some(&mut queue)).await?;
			...
			self.connection.send_message(&OutboundMessage::close(mailbox, mood), Some(&mut queue)).await?;
			match self.connection.receive_reply(Some(&mut queue)).await? {
