...
impl Wordlist {
	...
	pub fn get_completions(&self, prefix: &str) -> Vec<String> {
		...
		matches.iter().map(|word| format!("{prefix}-{word}")).collect()
	}
	...
	fn normal_complete<'a>(&self, partial: &str, words: &'a [&str]) -> Vec<&'a str> {
		words.iter().filter(|word| !partial.is_empty() && word.starts_with(partial)).copied().collect()
	}
	...
	pub fn default_wordlist(num_words: usize) -> Wordlist {
		Wordlist { num_words, words: load_pgpwords() }
	}
}
...
fn load_pgpwords() -> Vec<Vec<String>> {
	...
	for (index_str, values) in raw_words.iter() {
		...
		even_words[index] = values.get(1).unwrap().as_str().unwrap().to_lowercase().to_string();
		odd_words[index] = values.get(0).unwrap().as_str().unwrap().to_lowercase().to_string();
	}
...
mod test {
	...
	fn vec_strs(all: &str) -> Vec<&str> {
		all.split_whitespace().map(|s| if s == "." { "" } else { s }).collect()
	}
	...
	#[cfg_attr(target_arch = "wasm32", wasm_bindgen_test::wasm_bindgen_test)]
	fn test_completion() {
		let words: Vec<Vec<String>> = vec![vec_strings("purple green yellow"), vec_strings("sausages seltzer snobol")];
		...
		assert_eq!(w.get_completions("sausages-yello"), vec!["sausages-yellow".to_string()]);
	}
	...
	fn test_choose_words_matches_completion() {
		...
		assert_eq!(w.get_completions("1-purple-sausages").first().unwrap(), &format!("1-{}", w.choose_words().as_ref()));
	}
	...
	fn test_choose_more_words() {
		...
		let expected3 = ["purple-sausages-purple", "yellow-sausages-purple", "purple-sausages-yellow", "yellow-sausages-yellow"];
	...
	fn test_completion_fuzzy() {
		...
		assert_eq!(wl.fuzzy_complete("bzili", &list).first().unwrap(), &"brazilian");

		assert_eq!(wl.fuzzy_complete("carvan", &list).first().unwrap(), &"caravan");

		assert_ne!(wl.fuzzy_complete("choking", &list).first().unwrap(), &"choking")
	}
	...
	fn test_completion_normal() {
		...
		assert_eq!(wl.normal_complete("braz", &list).first().unwrap(), &"brazilian");
	...
	fn test_wormhole_code_normal_completions() {
		...
		assert_eq!(list.get_completions("22-compo").first().unwrap(), "22-component");

		assert_eq!(list.get_completions("22-component-check").first().unwrap(), "22-component-checkup");
	...
	fn test_wormhole_code_fuzzy_completions() {
		...
		assert_eq!(list.get_completions("22-udau").first().unwrap(), "22-undaunted");

		assert_eq!(list.get_completions("22-undua").first().unwrap(), "22-undaunted");

		assert_eq!(list.get_completions("22-undaunted-usht").first().unwrap(), "22-undaunted-upshot");
	}
}
