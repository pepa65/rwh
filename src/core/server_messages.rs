...
impl Nameplate_ {
	...
	where
		S: serde::Serializer,
	{
		ser.collect_seq(value.iter().map(|value| Self { id: value.to_string() }))
	}
}
...
impl PermissionRequired {
	...
	where
		D: serde::Deserializer<'de>,
	{
		let value: Option<serde_json::Map<String, serde_json::Value>> = serde::Deserialize::deserialize(de)?;
		Ok(value.is_some())
	}
}
...
impl std::fmt::Display for PermissionRequired {
	fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
		...
		write!(f, "PermissionRequired {{ one of: {:?}}}", none_iter.chain(hashcash_iter).chain(other_iter))
	}
}
...
#[display("EncryptedMessage {{ side: {}, phase: {}, body: {}", side, phase, "crate::util::DisplayBytes(body)")]
...
pub enum OutboundMessage {
	...
	#[display("Add {{ phase: {}, body: {} }}", phase, "crate::util::DisplayBytes(body)")]
	Add {
		phase: Phase,
...
impl OutboundMessage {
	...
	pub fn claim(nameplate: impl Into<String>) -> Self {
		OutboundMessage::Claim { nameplate: nameplate.into() }
	}

	pub fn release(nameplate: impl Into<String>) -> Self {
		OutboundMessage::Release { nameplate: nameplate.into() }
	}
...
mod test {
	...
	fn test_welcome3() {
		...
		assert!(matches!(
			m,
			InboundMessage::Welcome { welcome: WelcomeMessage { current_cli_version: None, motd: None, error: None, permission_required: None } }
		));
	}
			...
			InboundMessage::Welcome {
				welcome: WelcomeMessage {
					...
					permission_required: Some(PermissionRequired {
						none: true,
						hashcash: Some(HashcashPermission { bits: 6, resource: "resource-string".into() }),
						other: [("dark-ritual".to_string(), json!({ "hocrux": true }))].into_iter().collect()
					}),
	...
	fn test_submit_permissions() {
		let m = OutboundMessage::SubmitPermission(SubmitPermission::Hashcash { stamp: "stamp".into() });
		let s = serde_json::to_string(&m).unwrap();
		assert_eq!(s, r#"{"type":"submit-permission","method":"hashcash","stamp":"stamp"}"#);
	}
