...
use crate::transfer;
use crate::{self as magic_wormhole, AppConfig, AppID, Code, WormholeError, core::MailboxConnection, transit, util::timeout};
...
pub const TEST_APPID: AppID = AppID(std::borrow::Cow::Borrowed("magic-wormhole.github.io/magic-wormhole.rs/test"));

pub const APP_CONFIG: AppConfig<()> =
	AppConfig::<()> { id: TEST_APPID, rendezvous_url: Cow::Borrowed(crate::rendezvous::DEFAULT_RENDEZVOUS_SERVER), app_version: () };
...
fn default_relay_hints() -> Vec<transit::RelayHint> {
	vec![transit::RelayHint::from_urls(None, [transit::DEFAULT_RELAY_SERVER.parse().unwrap()]).unwrap()]
}
...
async fn test_connect_with_unknown_code_and_allocate_passes() {
	...
	let mailbox_connection = MailboxConnection::connect(transfer::APP_CONFIG.id(TEST_APPID).clone(), code, true).await;

	assert!(mailbox_connection.is_ok());

	mailbox_connection.unwrap().shutdown(Mood::Happy).await.unwrap()
}
...
async fn test_connect_with_unknown_code_and_no_allocate_fails() {
	...
	let mailbox_connection = MailboxConnection::connect(transfer::APP_CONFIG.id(TEST_APPID).clone(), code.clone(), false).await;
...
async fn file_offers() -> eyre::Result<Vec<(transfer::offer::OfferSend, transfer::offer::OfferAccept)>> {
	async fn offer(name: &str) -> eyre::Result<(transfer::offer::OfferSend, transfer::offer::OfferAccept)> {
		...
		let (data, offer) = {
			let data = match name {
				"example-file-4096.bin" => include_bytes!("../../tests/example-file-4096.bin").to_vec(),
			...
			let offer = transfer::offer::OfferSend::new_file_custom(
				...
				Box::new({
					...
					move || {
						let data = data.clone();
						Box::pin(async move {
							Ok(Box::new(async_io::Cursor::new(data)) as Box<dyn crate::transfer::offer::AsyncReadSeek + std::marker::Send + Unpin + 'static>)
						})
					}
				}),
			);
		...
		let (data, offer) = {
			...
			let offer = transfer::offer::OfferSend::new_file_or_folder(name.into(), &path).await.unwrap();
		...
		let answer = offer.set_content(|_path| {
			...
			let content = transfer::offer::new_accept_content({
				...
				move |_append| {
					...
					impl futures::io::AsyncWrite for Writer {
						...
						fn poll_close(mut self: Pin<&mut Self>, _: &mut Context<'_>) -> Poll<io::Result<()>> {
							...
							if self.send_bytes == self.receive_bytes {
								...
								Poll::Ready(Err(io::Error::other("Send and receive are not the same")))
							}
						}
					...
					async move { Ok(Writer { closed: false, send_bytes, receive_bytes: Vec::new() }) }
				}
			});
			transfer::offer::AcceptInner { content, offset: 0, sha256: None }
		});
...
async fn test_file_rust2rust() {
	for (offer, answer) in file_offers().await.unwrap() {
		...
		let sender_task = async {
			let mailbox_connection = MailboxConnection::create(transfer::APP_CONFIG.id(TEST_APPID).clone(), 2).await?;
		...
		let receiver_task = async {
			...
			let req = transfer::request(wormhole, default_relay_hints(), magic_wormhole::transit::Abilities::ALL, futures::future::pending())
				.await?
				.unwrap();

			req.accept(&log_transit_connection, |_received, _total| {}, answer, futures::future::pending()).await?;
			eyre::Result::<_>::Ok(())
		};
...
async fn test_send_many() {
	let mailbox = MailboxConnection::create(transfer::APP_CONFIG.id(TEST_APPID), 2).await.unwrap();
	...
	let senders = crate::util::spawn(async move {
		...
		for i in 1..5usize {
			...
			let wormhole = crate::Wormhole::connect(MailboxConnection::connect(transfer::APP_CONFIG.id(TEST_APPID), sender_code.clone(), true).await?).await?;
			senders.push(crate::util::spawn(async move {
				eyre::Result::Ok(
					crate::transfer::send(
						wormhole,
	...
	for i in 0..5usize {
		...
		req.accept(&log_transit_connection, |_, _| {}, gen_accept().await.unwrap(), futures::future::pending()).await.unwrap();
	}
...
async fn test_crowded() {
	...
	match futures::try_join!(mailbox_connection_1, mailbox_connection_2).err().unwrap() {
		magic_wormhole::WormholeError::ServerError(magic_wormhole::rendezvous::RendezvousError::Server(error)) => {
...
fn test_mood() {
	...
	// self._T.close())
	assert_eq!(String::from(r#""happy""#), serde_json::to_string(&Mood::Happy).unwrap());
	assert_eq!(String::from(r#""lonely""#), serde_json::to_string(&Mood::Lonely).unwrap());
	assert_eq!(String::from(r#""errory""#), serde_json::to_string(&Mood::Errory).unwrap());
	assert_eq!(String::from(r#""scary""#), serde_json::to_string(&Mood::Scared).unwrap());
	assert_eq!(String::from(r#""unwelcome""#), serde_json::to_string(&Mood::Unwelcome).unwrap());
}
