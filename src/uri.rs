...
impl WormholeTransferUri {
	...
	pub fn new(code: Code) -> Self {
		Self { code, rendezvous_server: None, is_leader: false }
	}
}
...
impl TryFrom<&url::Url> for WormholeTransferUri {
	...
	fn try_from(url: &url::Url) -> Result<Self, ParseError> {
		...
		let queries = url.query_pairs().collect::<std::collections::HashMap<_, _>>();
		...
		let rendezvous_server = queries.get("rendezvous").map(Deref::deref).map(url::Url::parse).transpose()?;
		...
		let code: Code = percent_encoding::percent_decode_str(url.path()).decode_utf8()?.parse().map_err(|e| {
			// TODO: Remove for 0.8
			if matches!(e, ParseCodeError::Empty) { ParseError::MissingCode } else { e.into() }
		})?;

		Ok(WormholeTransferUri { code, rendezvous_server, is_leader })
	}
}
...
mod test {
	...
	fn test_uri() {
		test_eq(WormholeTransferUri::new("4-hurricane-equipment".parse().unwrap()), "wormhole-transfer:4-hurricane-equipment");

		test_eq(WormholeTransferUri::new("8-🙈-🙉-🙊".parse().unwrap()), "wormhole-transfer:8-%F0%9F%99%88-%F0%9F%99%89-%F0%9F%99%8A");
	...
	fn test_uri_err() {
		assert_eq!(
			"wormhole-transfer:8-%F0%9F%99%88-%F0%9F%99%89-%F0%9F%99%8A?version=42&rendezvous=ws%3A%2F%2Flocalhost%3A4000%2F&role=leader"
				.parse::<WormholeTransferUri>(),
			Err(ParseError::UnsupportedVersion("42".into()))
		);
		assert_eq!(
			"wormhole-transfer:?rendezvous=ws%3A%2F%2Flocalhost%3A4000%2F&role=leader".parse::<WormholeTransferUri>(),
