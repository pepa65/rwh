...
impl Hints {
	...
	pub fn new(direct_tcp: impl IntoIterator<Item = DirectHint>, relay: impl IntoIterator<Item = RelayHint>) -> Self {
		Self { direct_tcp: direct_tcp.into_iter().collect(), relay: relay.into_iter().collect() }
	}
}
...
pub enum RelayHintParseError {
	#[error("Invalid TCP hint endpoint: '{}' (Does it have hostname and port?)", _0)]
	/// Invalid TCP hint endpoint
	InvalidTcp(url::Url),
	#[error("Unknown schema: '{}'. Currently known values are 'tcp', 'ws'  and 'wss'.", _0)]
...
impl RelayHint {
	...
	pub fn new(name: Option<String>, tcp: impl IntoIterator<Item = DirectHint>, ws: impl IntoIterator<Item = url::Url>) -> Self {
		Self { name, tcp: tcp.into_iter().collect(), ws: ws.into_iter().collect() }
	}
	...
	/// ```
	pub fn from_urls(name: Option<String>, urls: impl IntoIterator<Item = url::Url>) -> Result<Self, RelayHintParseError> {
		let mut this = Self { name, ..Self::default() };
		...
		assert!(!this.tcp.is_empty() || !this.ws.is_empty(), "No URLs provided");
		Ok(this)
	}
...
impl serde::Serialize for RelayHint {
		...
		S: serde::Serializer,
	{
		...
		hints.extend(self.ws.iter().cloned().map(|h| RelayHintSerdeInner::Websocket { url: h }));
...
impl<'de> serde::Deserialize<'de> for RelayHint {
		...
		D: serde::Deserializer<'de>,
	{
		...
		let mut hint = RelayHint { name: raw.name, tcp: HashSet::new(), ws: HashSet::new() };
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match &self.conn_type {
			ConnectionType::Direct => {
				write!(f, "Established direct transit connection to '{}'", self.peer_addr,)
			}
			ConnectionType::Relay { name: Some(name) } => {
				write!(f, "Established transit connection via relay '{}' ({})", name, self.peer_addr,)
			}
			ConnectionType::Relay { name: None } => {
				write!(f, "Established transit connection via relay ({})", self.peer_addr,)
			}
		}
	}
}
			...
			async {
					...
					.map_err(|_| StunError::Timeout)
				{
					Ok(Ok((external_ip, stream))) => {
						...
						our_hints.direct_tcp.insert(DirectHint { hostname: external_ip.ip().to_string(), port: external_ip.port() });
						tracing::debug!("Our socket for connecting is bound to {} and connected to {}", stream.local_addr()?, stream.peer_addr()?,);
						stream.into()
					}
					...
					Err(err) | Ok(Err(err)) => {
						...
						tracing::debug!("Our socket for connecting is bound to {}", socket.local_addr()?.as_socket().unwrap(),);
				...
				our_hints.direct_tcp.extend(if_addrs::get_if_addrs()?.iter().filter(|iface| !iface.is_loopback()).flat_map(|ip| {
					[DirectHint { hostname: ip.ip().to_string(), port }, DirectHint { hostname: ip.ip().to_string(), port: port2 }].into_iter()
				}));
...
impl TransitConnector {
	...
	pub async fn connect(
		self, role: TransitRole, transit_key: Key<TransitKey>, their_abilities: Abilities, their_hints: Arc<Hints>,
	) -> Result<(Transit, TransitInfo), TransitConnectError> {
		match role {
			TransitRole::Leader => self.leader_connect(transit_key, their_abilities, their_hints).await,
			TransitRole::Follower => self.follower_connect(transit_key, their_abilities, their_hints).await,
		}
	}
	...
	async fn leader_connect(
		...
		let (mut transit, mut finalizer, mut conn_info) = crate::util::timeout(std::time::Duration::from_secs(60), connection_stream.next())
			.await
			.map_err(|_| {
	...
	async fn follower_connect(
		...
		let transit = match crate::util::timeout(std::time::Duration::from_secs(60), &mut connection_stream.next()).await {
			Ok(Some((mut socket, finalizer, conn_info))) => {
				let (tx, rx) = finalizer.handshake_finalize(&mut socket).await.map_err(|e| {
					tracing::debug!("`handshake_finalize` failed: {e}");
					TransitConnectError::Handshake
				})?;
	...
	fn connect_inner(
		...
		let cryptor = if our_abilities.can_noise_crypto() && their_abilities.can_noise_crypto() {
			...
			Arc::new(crypto::NoiseInit { key: transit_key.clone() }) as Arc<dyn crypto::TransitCryptoInit>
		} else {
			tracing::debug!("Using secretbox for encryption");
			Arc::new(crypto::SecretboxInit { key: transit_key.clone() }) as Arc<dyn crypto::TransitCryptoInit>
		};
					...
					connectors.chain(
						relay_hints
							...
							.flat_map(|hint| {
								...
								hint.tcp.into_iter().take(3).enumerate().map(move |(i, h)| (i, h, name.clone()))
							})
							.map(|(index, host, name)| async move {
								async_io::Timer::after(std::time::Duration::from_secs(index as u64 * 5)).await;
					...
					connectors.chain(
						relay_hints
							...
							.flat_map(|hint| {
								...
								hint.ws.into_iter().take(3).enumerate().map(move |(i, u)| (i, u, name.clone()))
							})
							.map(|(index, url, name)| async move {
								crate::util::sleep(std::time::Duration::from_secs(index as u64 * 5)).await;
			...
			connectors
				.map(move |fut| {
					...
					async move {
						...
						let (transit, finalizer) = handshake_exchange(is_leader, tside, socket, &conn_info.conn_type, &*cryptor, transit_key).await?;
				...
				connectors.chain(
					std::iter::once(async move {
						...
						let connect = || async {
							...
							let (socket, info) = transport::wrap_tcp_connection(socket, ConnectionType::Direct)?;
							tracing::debug!("Got connection from {}!", peer);
							let (transit, finalizer) =
								handshake_exchange(is_leader, tside.clone(), socket, &ConnectionType::Direct, &*cryptor, transit_key.clone()).await?;
						...
						loop {
							match connect().await {
								...
								Err(err) => {
									tracing::debug!("Some handshake failed on the listening port: {:?}", err);
									continue;
								}
							}
						}
...
impl Transit {
	...
	pub fn split(
		...
		(
			futures::sink::unfold((writer, self.tx), |(mut writer, mut tx), plaintext: Box<[u8]>| async move {
				tx.encrypt(&mut writer, &plaintext).await.map(|()| (writer, tx))
			}),
			futures::stream::try_unfold((reader, self.rx), |(mut reader, mut rx)| async move {
				rx.decrypt(&mut reader).await.map(|record| Some((record, (reader, rx))))
			}),
		)
	}
}
...
async fn handshake_exchange(
...
) -> Result<(Box<dyn TransitTransport>, Box<dyn crypto::TransitCryptoInitFinalizer>), TransitHandshakeError> {
	if host_type != &ConnectionType::Direct {
		...
		socket.write_all(format!("please relay {} for side {}\n", sub_key.to_hex(), tside).as_bytes()).await?;
		let mut rx = [0u8; 3];
	...
	let finalizer = if is_leader { cryptor.handshake_leader(&mut socket).await? } else { cryptor.handshake_follower(&mut socket).await? };
...
mod test {
	...
	pub fn test_abilities_encoding() {
		assert_eq!(serde_json::to_value(Abilities::ALL).unwrap(), json!([{"type": "direct-tcp-v1"}, {"type": "relay-v1"}]));
		assert_eq!(serde_json::to_value(Abilities::FORCE_DIRECT).unwrap(), json!([{"type": "direct-tcp-v1"}]));
	}
	...
	pub fn test_hints_encoding() {
		assert_eq!(
			serde_json::to_value(Hints::new(
				[DirectHint { hostname: "localhost".into(), port: 1234 }],
				[RelayHint::new(
					Some("default".into()),
