...
use super::{TransitError, TransitKey, TransitRxKey, TransitTransport, TransitTransportRx, TransitTransportTx, TransitTxKey};
...
pub(super) trait TransitCryptoInitFinalizer: Send {
	fn handshake_finalize(self: Box<Self>, socket: &mut dyn TransitTransport) -> BoxFuture<'_, Result<DynTransitCrypto, TransitHandshakeError>>;
}
...
impl TransitCryptoInitFinalizer for DynTransitCrypto {
	fn handshake_finalize(self: Box<Self>, _socket: &mut dyn TransitTransport) -> BoxFuture<'_, Result<DynTransitCrypto, TransitHandshakeError>> {
...
pub(super) trait TransitCryptoInit: Send + Sync {
	...
	async fn handshake_leader(&self, socket: &mut dyn TransitTransport) -> Result<Box<dyn TransitCryptoInitFinalizer>, TransitHandshakeError>;
...
impl TransitCryptoInit for SecretboxInit {
	async fn handshake_leader(&self, socket: &mut dyn TransitTransport) -> Result<Box<dyn TransitCryptoInitFinalizer>, TransitHandshakeError> {
		...
		let rkey = self.key.derive_subkey_from_purpose("transit_record_receiver_key");
		let skey = self.key.derive_subkey_from_purpose("transit_record_sender_key");

		// for transmit mode, send send_handshake_msg and compare.
		// the received message with send_handshake_msg
		socket
			.write_all(format!("transit sender {} ready\n\n", self.key.derive_subkey_from_purpose::<GenericKey>("transit_sender").to_hex()).as_bytes())
			.await?;

		let expected_rx_handshake = format!("transit receiver {} ready\n\n", self.key.derive_subkey_from_purpose::<GenericKey>("transit_receiver").to_hex());
		...
		impl TransitCryptoInitFinalizer for Finalizer {
			fn handshake_finalize(self: Box<Self>, socket: &mut dyn TransitTransport) -> BoxFuture<Result<DynTransitCrypto, TransitHandshakeError>> {
				Box::pin(async move {
					socket.write_all(b"go\n").await?;

					Ok::<_, TransitHandshakeError>((
						Box::new(SecretboxCryptoEncrypt { skey: self.skey, snonce: Default::default() }) as Box<dyn TransitCryptoEncrypt>,
						Box::new(SecretboxCryptoDecrypt { rkey: self.rkey, rnonce: Default::default() }) as Box<dyn TransitCryptoDecrypt>,
					))
				})
			}
		}
	...
	async fn handshake_follower(&self, socket: &mut dyn TransitTransport) -> Result<Box<dyn TransitCryptoInitFinalizer>, TransitHandshakeError> {
		...
		// the received message with send_handshake_msg
		socket
			.write_all(format!("transit receiver {} ready\n\n", self.key.derive_subkey_from_purpose::<GenericKey>("transit_receiver").to_hex(),).as_bytes())
			.await?;

		let expected_tx_handshake = format!("transit sender {} ready\n\ngo\n", self.key.derive_subkey_from_purpose::<GenericKey>("transit_sender").to_hex(),);
		assert_eq!(expected_tx_handshake.len(), 90);
		socket.read_expect(expected_tx_handshake.as_bytes()).await?;

		Ok(Box::new((
			Box::new(SecretboxCryptoEncrypt { skey, snonce: Default::default() }) as Box<dyn TransitCryptoEncrypt>,
			Box::new(SecretboxCryptoDecrypt { rkey, rnonce: Default::default() }) as Box<dyn TransitCryptoDecrypt>,
		)) as Box<dyn TransitCryptoInitFinalizer>)
	}
}

type NoiseHandshakeState = noise_protocol::HandshakeState<noise_rust_crypto::X25519, noise_rust_crypto::ChaCha20Poly1305, noise_rust_crypto::Blake2s>;
...
impl TransitCryptoInit for NoiseInit {
	async fn handshake_leader(&self, socket: &mut dyn TransitTransport) -> Result<Box<dyn TransitCryptoInitFinalizer>, TransitHandshakeError> {
		socket.write_all(b"Magic-Wormhole Dilation Handshake v1 Leader\n\n").await?;
		socket.read_expect(b"Magic-Wormhole Dilation Handshake v1 Follower\n\n").await?;
		...
		// → psk, e
		socket.write_transit_message(&handshake.write_message_vec(&[])?).await?;
		...
		ensure!(peer_confirmation_message.is_empty(), TransitHandshakeError::HandshakeFailed);
		...
		impl TransitCryptoInitFinalizer for Finalizer {
			fn handshake_finalize(mut self: Box<Self>, socket: &mut dyn TransitTransport) -> BoxFuture<Result<DynTransitCrypto, TransitHandshakeError>> {
				Box::pin(async move {
					// → ""
					socket.write_transit_message(&self.tx.encrypt_vec(&[])).await?;

					Ok::<_, TransitHandshakeError>((
						Box::new(NoiseCryptoEncrypt { tx: self.tx }) as Box<dyn TransitCryptoEncrypt>,
						Box::new(NoiseCryptoDecrypt { rx: self.rx }) as Box<dyn TransitCryptoDecrypt>,
					))
				})
			}
		}
	...
	async fn handshake_follower(&self, socket: &mut dyn TransitTransport) -> Result<Box<dyn TransitCryptoInitFinalizer>, TransitHandshakeError> {
		socket.write_all(b"Magic-Wormhole Dilation Handshake v1 Follower\n\n").await?;
		socket.read_expect(b"Magic-Wormhole Dilation Handshake v1 Leader\n\n").await?;
...
pub(super) trait TransitCryptoEncrypt: Send {
	async fn encrypt(&mut self, socket: &mut dyn TransitTransportTx, plaintext: &[u8]) -> Result<(), TransitError>;
}

#[async_trait]
pub(super) trait TransitCryptoDecrypt: Send {
	async fn decrypt(&mut self, socket: &mut dyn TransitTransportRx) -> Result<Box<[u8]>, TransitError>;
}
...
impl TransitCryptoEncrypt for SecretboxCryptoEncrypt {
	async fn encrypt(&mut self, socket: &mut dyn TransitTransportTx, plaintext: &[u8]) -> Result<(), TransitError> {
		...
		socket.write_all(&((ciphertext.len() + nonce.len()) as u32).to_be_bytes()).await?;
...
impl TransitCryptoDecrypt for SecretboxCryptoDecrypt {
	async fn decrypt(&mut self, socket: &mut dyn TransitTransportRx) -> Result<Box<[u8]>, TransitError> {
		...
		use std::io::{Error, ErrorKind};
			...
			Error::new(ErrorKind::InvalidData, "Message must be long enough to contain at least the nonce")
		);

		// 3. decrypt the vector 'enc_packet' with the key.
		let plaintext = {
			let (received_nonce, ciphertext) = enc_packet.split_at(secretbox::SecretBox::<secretbox::XSalsa20Poly1305>::NONCE_SIZE);
			{
				// Nonce check
				ensure!(nonce.as_slice() == received_nonce, TransitError::Nonce(received_nonce.into(), nonce.as_slice().into()),);
...
impl TransitCryptoEncrypt for NoiseCryptoEncrypt {
	async fn encrypt(&mut self, socket: &mut dyn TransitTransportTx, plaintext: &[u8]) -> Result<(), TransitError> {
		socket.write_transit_message(&self.tx.encrypt_vec(plaintext)).await?;
		Ok(())
	}
}
