...
pub(super) trait TransitTransportRx: AsyncRead + std::any::Any + Unpin + Send {
	...
	async fn read_transit_message(&mut self) -> Result<Vec<u8>, std::io::Error> {
		...
		ensure!(len == length, Error::new(ErrorKind::UnexpectedEof, "failed to read whole message"));
		Ok(buffer)
	}
}
...
pub(super) trait TransitTransportTx: AsyncWrite + std::any::Any + Unpin + Send {
	...
	async fn write_transit_message(&mut self, message: &[u8]) -> Result<(), std::io::Error> {
		...
		self.write_all(&(message.len() as u32).to_be_bytes()).await?;
...
pub(super) fn set_socket_opts(socket: &socket2::Socket) -> std::io::Result<()> {
	...
	#[cfg(not(any(all(unix, not(any(target_os = "solaris", target_os = "illumos"))), target_os = "windows")))]
	{
...
pub(super) async fn tcp_get_external_ip() -> Result<(SocketAddr, TcpStream), StunError> {
	...
	fn get_binding_request() -> Result<Vec<u8>, bytecodec::Error> {
		...
		let mut message: Message<Attribute> = Message::new(MessageClass::Request, rfc5389::methods::BINDING, TransactionId::new(random_bytes));

		message.add_attribute(Attribute::Software(Software::new("magic-wormhole-rust".to_owned())?));
	...
	let external_addr = decode_address(&buf[..20 + len as usize])?.ok_or(StunError::ServerNoResponse)?;

	Ok((external_addr, socket))
}

/**
 * Bind to a port with SO_REUSEADDR, connect to the destination and then hide the blood behind a pretty [`async_net::TcpStream`]
 *
 * We want an `async_net::TcpStream`, but with SO_REUSEADDR set.
 ...
 */
#[cfg(not(target_family = "wasm"))]
async fn tcp_connect_custom(local_addr: &socket2::SockAddr, dest_addr: &socket2::SockAddr) -> std::io::Result<async_net::TcpStream> {
	...
	stream.get_ref().take_error().and_then(|maybe_err| maybe_err.map_or(Ok(()), Result::Err))?;
	/* Convert our mess to `async_net::TcpStream */
	stream.into_inner()?.try_into()
}

#[cfg(not(target_family = "wasm"))]
pub(super) async fn connect_tcp_direct(local_addr: Option<Arc<socket2::SockAddr>>, hint: DirectHint) -> Result<TransitConnection, TransitHandshakeError> {
	...
	if let Some(local_addr) = local_addr {
		...
		socket = async_net::TcpStream::connect(&dest_addr).await?;
		tracing::debug!("Connected to {}!", dest_addr);
	}
...
#[cfg(not(target_family = "wasm"))]
pub(super) async fn connect_tcp_relay(host: DirectHint, name: Option<String>) -> Result<TransitConnection, TransitHandshakeError> {
	tracing::debug!("Connecting to relay {}", host);
	let socket = TcpStream::connect((host.hostname.as_str(), host.port)).err_into::<TransitHandshakeError>().await?;
...
pub(super) async fn connect_ws_relay(url: url::Url, name: Option<String>) -> Result<TransitConnection, TransitHandshakeError> {
	tracing::debug!("Connecting to relay {}", url);
	let (_meta, transit) = ws_stream_wasm::WsMeta::connect(&url, None).err_into::<TransitHandshakeError>().await?;
	tracing::debug!("Connected to {}!", url);

	let transit = Box::new(transit.into_io()) as Box<dyn TransitTransport>;

	Ok((transit, TransitInfo { conn_type: ConnectionType::Relay { name } }))
}

/* Take a tcp connection and transform it into a `TransitConnection` (mainly set timeouts) */
#[cfg(not(target_family = "wasm"))]
pub(super) fn wrap_tcp_connection(socket: TcpStream, conn_type: ConnectionType) -> Result<TransitConnection, TransitHandshakeError> {
	...
	let async_stream: Arc<async_io::Async<std::net::TcpStream>> = socket.into();
	...
	let info = TransitInfo { conn_type, peer_addr: socket.peer_addr().expect("Internal error: socket must be IP") };
