...
impl TransitAck {
	pub(crate) fn new(msg: impl Into<String>, sha256: impl Into<String>) -> Self {
		TransitAck { ack: msg.into(), sha256: sha256.into() }
	}
...
pub(crate) async fn send(
	wormhole: Wormhole, relay_hints: Vec<transit::RelayHint>, transit_abilities: transit::Abilities, offer: OfferSend,
	progress_handler: impl FnMut(u64, u64) + 'static, transit_handler: impl FnOnce(transit::TransitInfo), _peer_version: AppVersion,
	cancel: impl Future<Output = ()>,
) -> Result<(), TransferError> {
	if offer.is_multiple() {
		let folder = OfferSendEntry::Directory { content: offer.content };
		send_folder(wormhole, relay_hints, "<unnamed folder>".into(), folder, transit_abilities, transit_handler, progress_handler, cancel).await
	} else if offer.is_directory() {
		let (folder_name, folder) = offer.content.into_iter().next().unwrap();
		send_folder(wormhole, relay_hints, folder_name, folder, transit_abilities, transit_handler, progress_handler, cancel).await
	} else {
...
pub(crate) async fn send_file<F, G, H>(
	mut wormhole: Wormhole, relay_hints: Vec<transit::RelayHint>, file: &mut F, file_name: impl Into<String>, file_size: u64,
	transit_abilities: transit::Abilities, transit_handler: G, progress_handler: H, cancel: impl Future<Output = ()>,
) -> Result<(), TransferError>
where
	...
	H: FnMut(u64, u64) + 'static,
{
	let run = Box::pin(async {
		...
		wormhole.send_json(&PeerMessage::offer_file_v1(file_name, file_size)).await?;

		// Wait for their transit response
		let (their_abilities, their_hints): (transit::Abilities, transit::Hints) = match wormhole.receive_json::<PeerMessage>().await??.check_err()? {
...

		{
			...
			match fileack_msg.check_err()? {
				...
				_ => {
					bail!(TransferError::unexpected_message("answer/file_ack", fileack_msg));
				}
			}
		}

		let (mut transit, info) = connector
			.connect(TransitRole::Leader, wormhole.key().derive_transit_key(wormhole.appid()), their_abilities, Arc::new(their_hints))
			.await?;
		...
		let file = futures::stream::once(futures::future::ready(std::io::Result::Ok(Box::new(file) as Box<dyn AsyncRead + Unpin + Send>)));
...
pub(crate) async fn send_folder(
	mut wormhole: Wormhole, relay_hints: Vec<transit::RelayHint>, mut folder_name: String, folder: OfferSendEntry, transit_abilities: transit::Abilities,
	...
	let run = Box::pin(async {
			...
			match offer {
				...
				OfferSendEntry::RegularFile { size, content } => {
					...
					*total_size += size;
					...
					total_content.push(wrap(header));
					let content =
						content().map_ok(/* Re-box because we can't upcast trait objects */ |read| Box::new(read) as Box<dyn AsyncRead + Unpin + Send>);
		...
		wormhole.send_json(&PeerMessage::offer_file_v1(folder_name, total_size)).await?;
		...
		// 11. send the file as encrypted records.
		let checksum = v1::send_records(&mut transit, content, total_size, progress_handler).await?;
...
pub async fn request(
	...
	let run = Box::pin(async {
		...
		let (filename, filesize) = match wormhole.receive_json::<PeerMessage>().await??.check_err()? {
			PeerMessage::Offer(offer_type) => match offer_type {
				...
				v1::OfferMessage::Directory { mut dirname, zipsize, .. } => {
	...
	futures::pin_mut!(cancel);
	let result = cancel::cancellable_2(run, cancel).await;
	cancel::handle_run_result_noclose(wormhole, result).await.map(|inner: Option<_>| {
		inner.map(|((filename, filesize, connector, their_abilities, their_hints), wormhole, _)| {
			ReceiveRequest::new(filename, filesize, connector, their_abilities, their_hints, wormhole)
		})
	})
}
...
impl ReceiveRequest {
	fn new(
		file_name: String, filesize: u64, connector: TransitConnector, their_abilities: transit::Abilities, their_hints: transit::Hints, wormhole: Wormhole,
	) -> Self {
		let their_hints = Arc::new(their_hints);
		let mut content = BTreeMap::new();

		// Synthesize an offer to make transfer v1 more similar to transfer v2
		content.insert(file_name.clone(), OfferEntry::RegularFile { size: filesize, content: () });
		...
		W: AsyncWrite + Unpin,
	{
		let run = Box::pin(async {
			...
			self.wormhole.send_json(&PeerMessage::file_ack_v1("ok")).await?;
			...
			tracing::debug!("Beginning file transfer");
			tcp_file_receive(&mut transit, self.filesize, progress_handler, content_handler).await?;
			Ok(())
		});
...
pub(crate) async fn send_records<'a>(
	transit: &mut Transit, files: impl futures::Stream<Item = std::io::Result<Box<dyn AsyncRead + Unpin + Send + 'a>>>, file_size: u64,
...
pub(crate) async fn tcp_file_receive<F, W>(transit: &mut Transit, filesize: u64, progress_handler: F, content_handler: &mut W) -> Result<(), TransferError>
where
	F: FnMut(u64, u64) + 'static,
	W: AsyncWrite + Unpin,
{
...
mod tar_helper {
	...
	pub(crate) fn padding(size: u64) -> &'static [u8] {
		...
		if !size.is_multiple_of(512) { &BLOCK[size as usize % 512..] } else { &[] }
	}

	fn append(mut dst: &mut dyn std::io::Write, header: &tar::Header, mut data: &mut dyn std::io::Read) -> std::io::Result<()> {
	...
	fn prepare_header_path(dst: &mut dyn std::io::Write, header: &mut tar::Header, path: &str) -> std::io::Result<()> {
