...
impl PeerMessageV2 {
	pub fn ser_msgpack(&self) -> Vec<u8> {
		...
		let mut ser = rmp_serde::encode::Serializer::new(&mut writer).with_struct_map().with_human_readable();
...
async fn make_transit(
	wormhole: &mut Wormhole, role: TransitRole, relay_hints: Vec<transit::RelayHint>, transit_abilities: transit::Abilities, peer_abilities: transit::Abilities,
	...
	wormhole.send_json(&PeerMessage::transit_v2((**connector.our_hints()).clone())).await?;

	/* Receive their transit hints */
	let their_hints: transit::Hints = match wormhole.receive_json::<PeerMessage>().await??.check_err()? {
		...
		other => {
			...
			let _ = wormhole.send_json(&PeerMessage::Error(format!("{error}"))).await;
			bail!(error)
		}
	};
	...
	let (transit, info) = match connector
		.connect(role, wormhole.key().derive_transit_key(wormhole.appid()), peer_abilities, Arc::new(their_hints))
		.await
	{
		Ok(transit) => transit,
		Err(error) => {
...
pub async fn send(
	mut wormhole: Wormhole, relay_hints: Vec<transit::RelayHint>, transit_abilities: transit::Abilities, offer: OfferSend,
	progress_handler: impl FnMut(u64, u64) + 'static, peer_version: AppVersion, cancel: impl Future<Output = ()>,
	...
	let (mut transit, wormhole, cancel) = cancel::with_cancel_wormhole!(
		wormhole,
		run = async {
			Ok(make_transit(&mut wormhole, TransitRole::Leader, relay_hints, transit_abilities, peer_abilities.transit_abilities)
				.await?
				.0)
		},
		cancel,
...
async fn send_inner(transit: &mut transit::Transit, offer: OfferSend, mut progress_handler: impl FnMut(u64, u64) + 'static) -> Result<(), TransferError> {
	transit
		.send_record(&{
	...
	for file in &files {
		if let Some((_, size)) = offer.get_file(&file.file) {
			...
			bail!(TransferError::Protocol(format!("Invalid file request: {}", file.file.join("/")).into()));
		}
	}
	...
	for AnswerMessageInner { file, offset, sha256 } in &files {
		...
		if let Some(sha256) = sha256 {
			...
			if *our_hash == sha256[..] {
				transit.send_record(&PeerMessageV2::FileStart(FileStart { file, start_at_offset: true }).ser_msgpack()).await?;
			} else {
				transit.send_record(&PeerMessageV2::FileStart(FileStart { file, start_at_offset: false }).ser_msgpack()).await?;
		...
		loop {
			...
			transit.send_record(&PeerMessageV2::Payload(Payload { payload: buffer.into() }).ser_msgpack()).await?;
			total_sent += n as u64;
			progress_handler(total_sent, total_size);
		...
		transit.send_record(&PeerMessageV2::FileEnd(FileEnd {}).ser_msgpack()).await?;
	}
	transit.send_record(&PeerMessageV2::TransferAck(TransferAck {}).ser_msgpack()).await?;

	Ok(())
}

pub async fn request(
	mut wormhole: Wormhole, relay_hints: Vec<transit::RelayHint>, peer_version: AppVersion, transit_abilities: transit::Abilities,
	...
	let ((mut transit, info), wormhole, cancel) = cancel::with_cancel_wormhole!(
		wormhole,
		run = async { make_transit(&mut wormhole, TransitRole::Follower, relay_hints, transit_abilities, peer_abilities.transit_abilities,).await },
		cancel,
		ret_cancel = None,
	);
...
impl ReceiveRequest {
	pub fn new(transit: Transit, offer: Offer, info: transit::TransitInfo) -> Self {
		Self { transit, offer: Arc::new(offer), info }
	}
	 ...
	 */
	pub async fn accept(
		self, transit_handler: impl FnOnce(transit::TransitInfo), answer: OfferAccept, progress_handler: impl FnMut(u64, u64) + 'static,
		cancel: impl Future<Output = ()>,
	) -> Result<(), TransferError> {
				...
				transit
					.send_record(&{
						...
						PeerMessageV2::Answer(AnswerMessage {
							files: answer
								.iter_files()
								.map(|(path, inner, _size)| AnswerMessageInner { file: path, offset: inner.offset, sha256: inner.sha256 })
								.collect(),
						})
						.ser_msgpack()
					})
	...
	pub async fn reject(mut self) -> Result<(), TransferError> {
		self.transit.send_record(&PeerMessageV2::Error("transfer rejected".into()).ser_msgpack()).await?;
...
async fn receive_inner(
	transit: &mut transit::Transit, offer: &Arc<Offer>, our_answer: OfferAccept, mut progress_handler: impl FnMut(u64, u64) + 'static,
) -> Result<(), TransferError> {
	 ...
	 */
	assert!(
		our_answer.iter_file_paths().all(|path| offer.get_file(&path).is_some()),
		"Mismatch between offer and accept: accept must be a true subset of offer"
	);
	let n_accepted = our_answer.iter_file_paths().count();
	let total_size = our_answer.iter_files().map(|(_path, _inner, size)| size).sum::<u64>();
	let mut total_received = 0;

	/* The receive loop */
	for (i, (file, answer, size)) in our_answer.into_iter_files().enumerate() {
		let file_start = match PeerMessageV2::de_msgpack(&transit.receive_record().await?)?.check_err()? {
			PeerMessageV2::FileStart(file_start) => file_start,
			PeerMessageV2::TransferAck(_) => {
				bail!(TransferError::Protocol(
					format!("Unexpected message: got 'transfer-ack' but expected {} more 'file-start' messages", n_accepted - i).into_boxed_str()
				))
			}
			other => {
			...
			TransferError::Protocol(format!("Unexpected file: got file {} but expected {}", file_start.file.join("/"), file.join("/"),).into_boxed_str())
		);
		...
		loop {
			let payload = match PeerMessageV2::de_msgpack(&transit.receive_record().await?)?.check_err()? {
				...
				PeerMessageV2::FileEnd(_) => {
					bail!(TransferError::Protocol(
						format!("Unexpected message: got 'file-end' but expected {} more payload bytes", size - received_size,).into_boxed_str()
					))
				}
				other => {
			...
			if received_size == size {
				 ...
				 */
				bail!(TransferError::Protocol(
					format!("File too large: expected only {size} bytes, got at least {} more", size - received_size).into_boxed_str()
				))
			}
		}
	...
	let _transfer_ack = match PeerMessageV2::de_msgpack(&transit.receive_record().await?)?.check_err()? {
		PeerMessageV2::TransferAck(transfer_ack) => transfer_ack,
		PeerMessageV2::FileStart(_) => {
			bail!(TransferError::Protocol(
				"Unexpected message: got 'file-start' but did not expect any more files".to_string().into_boxed_str()
			))
		}
		other => {
