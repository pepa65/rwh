...
impl OfferSend {
	...
	#[cfg(not(target_family = "wasm"))]
	pub async fn new_file_or_folder(offer_name: String, path: impl AsRef<Path>) -> std::io::Result<Self> {
		let path = path.as_ref();
		tracing::trace!("OfferSend::new_file_or_folder: {offer_name}, {}", path.display());
	...
	pub async fn new_paths(paths: impl IntoIterator<Item = PathBuf>) -> std::io::Result<Self> {
		...
		for path in paths {
			...
			let offer_name = offer_name
				.to_str()
				.ok_or_else(|| std::io::Error::other(format!("{} is not UTF-8 encoded", (offer_name.as_ref() as &Path).display())))?
				.to_owned();
...
impl<T> Offer<T> {
	...
	pub fn get_file(&self, path: &[String]) -> Option<(&T, u64)> {
		match path {
			[] => None,
			[start, rest @ ..] => self.content.get(start).and_then(|inner| inner.get_file(rest)),
		}
	}
	...
	pub fn accept_all(&self, target_dir: &Path) -> OfferAccept {
		self.set_content(|path| {
			...
			let content = new_accept_content(move |append| {
				...
				async_fs::OpenOptions::new().write(true).create(true).append(append).truncate(!append).open(full_path)
			});
			AcceptInner { content: Box::new(content) as _, offset: 0, sha256: None }
		})
	}
	...
	pub fn offer_name(&self) -> String {
		...
		if self.is_multiple() {
			format!("{name} and {} other files or directories", self.content.len() - 1)
	...
	pub fn is_directory(&self) -> bool {
		self.is_multiple() || self.content.values().any(|f| matches!(f, OfferEntry::Directory { .. }))
	}

	pub fn set_content<U>(&self, mut f: impl FnMut(&[String]) -> U) -> Offer<U> {
		Offer { content: self.content.iter().map(|(k, v)| (k.clone(), v.set_content(&mut vec![k.clone()], &mut f))).collect() }
	}
}
...
pub type OfferContent = Box<dyn Fn() -> futures::future::BoxFuture<'static, std::io::Result<Box<dyn AsyncReadSeek + Unpin + Send>>> + Send>;
			...
			let content: BTreeMap<String, Self> = async_fs::read_dir(path)
				...
				.and_then(|file| async move {
					...
					let name = path
						...
						.to_str()
						.ok_or_else(|| std::io::Error::other(format!("{} is not UTF-8 encoded", path.display())))?
						.to_owned();
...
impl<T> OfferEntry<T> {
	...
	fn iter_files(&self) -> impl Iterator<Item = (Vec<String>, &T, u64)> + '_ {
		...
		match self {
			...
			Self::RegularFile { content, size } => Box::new(std::iter::once((vec![], content, *size))) as Box<dyn Iterator<Item = _>>, // Self::Symlink { .. } => Box::new(std::iter::empty()) as Box<dyn Iterator<Item = _>>,
		}
	}
	...
	fn get(&self, path: &[String]) -> Option<&Self> {
		match path {
			...
			[start, rest @ ..] => match self {
				Self::Directory { content, .. } => content.get(start).and_then(|inner| inner.get(rest)),
				_ => None,
			},
		}
	}
	...
	fn get_file(&self, path: &[String]) -> Option<(&T, u64)> {
		match path {
			...
			[start, rest @ ..] => match self {
				Self::Directory { content, .. } => content.get(start).and_then(|inner| inner.get_file(rest)),
				_ => None,
			},
		}
	}
	...
	async fn create_directories(&self, target_path: &Path) -> std::io::Result<()> {
		#[inline(always)]
		fn recurse<'a, T>(this: &'a OfferEntry<T>, path: &'a Path) -> futures::future::LocalBoxFuture<'a, std::io::Result<()>> {
	...
	fn set_content<U>(&self, base_path: &mut Vec<String>, f: &mut impl FnMut(&[String]) -> U) -> OfferEntry<U> {
		match self {
			OfferEntry::RegularFile { size, .. } => OfferEntry::RegularFile { size: *size, content: f(base_path) },
...
impl<T: 'static + Send> OfferEntry<T> {
	...
	fn into_iter_files(self) -> impl Iterator<Item = (Vec<String>, T, u64)> + Send {
		...
		match self {
			...
			Self::RegularFile { content, size } => Box::new(std::iter::once((vec![], content, size))) as Box<dyn Iterator<Item = _> + Send>, // Self::Symlink { .. } => {
...
pub type AcceptContent = Box<dyn FnOnce(bool) -> futures::future::BoxFuture<'static, std::io::Result<Box<dyn AsyncWrite + Unpin + Send>>> + Send>;
