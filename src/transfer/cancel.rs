...
pub async fn cancellable<T>(future: impl Future<Output = T> + Unpin, cancel: impl Future<Output = ()>) -> Result<T, Cancelled> {
...
pub async fn cancellable_2<T, C: Future<Output = ()> + Unpin>(future: impl Future<Output = T> + Unpin, cancel: C) -> Result<(T, C), Cancelled> {
	use futures::future::Either;
	match futures::future::select(cancel, future).await {
		Either::Left(((), _)) => Err(Cancelled),
...
macro_rules! with_cancel_wormhole {
	($wormhole:ident, run = $run:expr_2021, $cancel:expr_2021, ret_cancel = $ret_cancel:expr_2021 $(,)?) => {{
		...
		let Some((transit, wormhole, cancel)) = cancel::handle_run_result_noclose($wormhole, result).await? else {
...
pub async fn handle_run_result(
	wormhole: Wormhole, result: Result<(Result<(), TransferError>, impl Future<Output = ()>), Cancelled>,
...
pub async fn handle_run_result_noclose<T, C: Future<Output = ()>>(
	...
	match result {
		...
		Ok((Err(error @ TransferError::PeerError(_)), cancel)) => {
			tracing::debug!("Transfer encountered an error ({}), doing cleanup logic", error);
			wrap_timeout(
				async {
					debug_err(wormhole.close().await, "close Wormhole");
				},
		...
		Ok((Err(mut error @ TransferError::Transit(_)), cancel)) => {
			...
			wrap_timeout(
				async {
					...
					match timeout(SHUTDOWN_TIME / 3, wormhole.receive_json()).await {
						Ok(Ok(Ok(PeerMessage::Error(e)))) => {
		...
		Ok((Err(error), cancel)) => {
			...
			wrap_timeout(
				async {
					debug_err(wormhole.send_json(&PeerMessage::Error(format!("{error}"))).await, "notify peer about the error");
					debug_err(wormhole.close().await, "close Wormhole");
				},
				cancel,
			)
		...
		Err(cancelled) => {
			...
			wrap_timeout(
				async {
					debug_err(wormhole.send_json(&PeerMessage::Error(format!("{cancelled}"))).await, "notify peer about our cancellation");
					debug_err(wormhole.close().await, "close Wormhole");
				},
...
pub async fn handle_run_result_transit<T>(
	mut transit: transit::Transit, result: Result<(Result<T, TransferError>, impl Future<Output = ()>), Cancelled>,
	make_error_message: impl FnOnce(&(dyn std::string::ToString + Sync)) -> Vec<u8>, parse_message: impl Fn(&[u8]) -> Result<Option<String>, TransferError>,
	...
	match result {
		...
		Ok((Err(error), cancel)) => {
			...
			wrap_timeout(
				async {
					debug_err(transit.send_record(&make_error_message(&error)).await, "notify peer about the error");
				},
				cancel,
			)
			.await;
		...
		Err(cancelled) => {
			...
			wrap_timeout(
				async {
					debug_err(transit.send_record(&make_error_message(&cancelled)).await, "notify peer about our cancellation");
				},
				futures::future::pending(),
			)
			.await;
