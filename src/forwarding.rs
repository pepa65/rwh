...
impl ForwardingError {
	...
	pub(self) fn unexpected_message(expected: impl Into<Box<str>>, got: impl std::fmt::Debug + Send + Sync + 'static) -> Self {
...
/// as the value.
pub async fn serve(
	mut wormhole: Wormhole, transit_handler: impl FnOnce(transit::TransitInfo), relay_hints: Vec<transit::RelayHint>, targets: Vec<(Option<url::Host>, u16)>,
	cancel: impl Future<Output = ()>,
) -> Result<(), ForwardingError> {
	assert!(!targets.is_empty(), "The list of target ports must not be empty");

	let our_version: &AppVersion = wormhole.our_version().downcast_ref().expect("You may only use a Wormhole instance with the correct AppVersion type!");
		...
		.map(|(host, port)| match host {
			Some(host) => {
				if port == 80 || port == 443 || port == 8000 || port == 8080 {
					tracing::warn!(
						"It seems like you are trying to forward a remote HTTP target ('{}'). Due to HTTP being host-aware this will very likely fail!",
						host
					);
				}
	...
	match result {
		...
		Err(error) => {
			let _ = transit_tx.send(PeerMessage::Error(format!("{error}")).ser_msgpack().into_boxed_slice()).await;
			Err(error)
		}
	}
}
...
impl ForwardingServe {
	async fn forward(
		&mut self, transit_tx: &mut (impl futures::sink::Sink<Box<[u8]>, Error = TransitError> + Unpin), connection_id: u64, payload: &[u8],
	) -> Result<(), ForwardingError> {
		...
		match self.connections.get_mut(&connection_id) {
			...
			None if !self.historic_connections.contains(&connection_id) => {
				bail!(ForwardingError::protocol(format!("Connection '{connection_id}' not found")));
			}
	...
	async fn remove_connection(
		&mut self, transit_tx: &mut (impl futures::sink::Sink<Box<[u8]>, Error = TransitError> + Unpin), connection_id: u64, tell_peer: bool,
	) -> Result<(), ForwardingError> {
		tracing::debug!("Removing connection: #{}", connection_id);
		if tell_peer {
			transit_tx.send(PeerMessage::Disconnect { connection_id }.ser_msgpack().into_boxed_slice()).await?;
		}
	...
	async fn spawn_connection(
		&mut self, transit_tx: &mut (impl futures::sink::Sink<Box<[u8]>, Error = TransitError> + Unpin), mut target: String, connection_id: u64,
	) -> Result<(), ForwardingError> {
		...
		let entry = match self.connections.entry(connection_id) {
			...
			Entry::Occupied(_) => {
				bail!(ForwardingError::protocol(format!("Connection '{connection_id}' already exists")));
			}
		};
		...
		let stream = match async_net::TcpStream::connect(&target).await {
			Ok(stream) => stream,
			Err(err) => {
				tracing::warn!("Cannot open connection to {}: {}. The forwarded service might be down.", target, err);
				transit_tx.send(PeerMessage::Disconnect { connection_id }.ser_msgpack().into_boxed_slice()).await?;
				return Ok(());
			}
		};
	...
	async fn run(
		mut self, transit_tx: &mut (impl futures::sink::Sink<Box<[u8]>, Error = TransitError> + Unpin),
...
pub async fn connect(
	mut wormhole: Wormhole, transit_handler: impl FnOnce(transit::TransitInfo), relay_hints: Vec<transit::RelayHint>, bind_address: Option<std::net::IpAddr>,
	custom_ports: &[u16],
	...
	match run.await {
		Ok(listeners) => Ok(ConnectOffer { transit, mapping: listeners.iter().map(|(_, b, c)| (*b, c.clone())).collect(), listeners }),
		Err(error @ ForwardingError::PeerError(_)) => Err(error),
		Err(error) => {
			let _ = transit.send_record(&PeerMessage::Error(format!("{error}")).ser_msgpack()).await;
			Err(error)
		}
	}
}
...
pub struct ConnectOffer {
	...
	listeners: Vec<(async_net::TcpListener, u16, std::rc::Rc<std::string::String>)>,
}
...
where
	I: Unpin + futures::stream::FusedStream<Item = Result<(Rc<String>, async_net::TcpStream), std::io::Error>>,
{
	async fn forward(
		&mut self, transit_tx: &mut (impl futures::sink::Sink<Box<[u8]>, Error = TransitError> + Unpin), connection_id: u64, payload: &[u8],
	) -> Result<(), ForwardingError> {
	...
	async fn spawn_connection(
		&mut self, transit_tx: &mut (impl futures::sink::Sink<Box<[u8]>, Error = TransitError> + Unpin), target: Rc<String>, connection: async_net::TcpStream,
	) -> Result<(), ForwardingError> {
		...
		transit_tx
			.send(PeerMessage::Connect { target: (*target).clone(), connection_id }.ser_msgpack().into_boxed_slice())
			.await?;
		...
		self.connections.insert(connection_id, (worker, connection_wr));
		Ok(())
	}
